  ...
  useEffect(() => {
    ...
    socketRef.current.on("receiveMessage", (message) => {
      setMessages((prev) => {
        ...
        // If matching temp message exists → replace it
        if (tempIndex !== -1) {
          const updated = [...prev];
        ...
        <div>
          {filePreviewUrl && selectedFile?.type.startsWith("image") && (
            <img
              src={filePreviewUrl}
              alt="preview"
              className="max-w-xs max-h-40 mt-2 rounded"
            />
          )}
          ...
          {selectedFile &&
            ...
            !selectedFile.type.startsWith("video") && (
              <div className="mt-2 text-xs text-gray-600 border rounded px-2 py-1">
                {selectedFile.name}
              </div>
            )}
        </div>
